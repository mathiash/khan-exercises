jQuery.extend(KhanUtil, {
	Polynomial: function( minDegree, maxDegree, coefs, variable, name ) {
		var term = function( coef, vari, degree ) {
			...
			if ( degree === 0 ) {
				...
				return {op:"*", params:[coef, vari]};
			} else {
				return {op:"*", params:[coef, {op:"^", params:[vari, degree]}]};
			}
		...
		var extractFromExpr = function ( expr ){
			...
			if ( typeof expr === "number" ){
				coef = expr;
				degree = 0;
			} else if ((typeof expr === "object") && (typeof expr.params[1] !== "object")){
				coef = expr.params[0];
				degree = 1;
			} else if ((typeof expr === "object") && (typeof expr.params[1] === "object")){
				coef = expr.params[0];
				degree = expr.params[1].params[1];
			}
			return {
				coef: coef,
				degree: degree
		...
		this.getNumberOfTerms = function() {
			return this.expr().params.length;
		...
		this.getCoefAndDegreeForTerm = function( termIndex ) {
			...
			return extractFromExpr( this.expr().params[termIndex] );

		};

		this.text = function() {
			return KhanUtil.exprToString( this.expr( this.variable ) );
		};

		this.toString = this.text;

		this.hintEvalOf = function( val ) {
			return KhanUtil.exprToString( this.expr( val ) );
		};

		this.evalOf = function( val ) {
			return KhanUtil.exprToString( this.expr( val ), true );
		};
	...
	CompositePolynomial: function( minDegree, maxDegree, coefs, variable, name,
		...
		var tackOn = function( expr, tack ) {
				...
				expr.params.push( tack );
			} else {
				expr = { op:"+", params:[expr, tack]};
			}
			return expr;
		};

		this.expr = function( vari ) {
			return tackOn( base.expr( vari ), {op:"*", params:[composedCoef, composedFunc]} );
		};
		...
		this.hintEvalOf = function( val, evalInner ) {
			if ( evalInner ) {
				...
				return KhanUtil.exprToString( tackOn( base.expr( val ),
					{op:"*", params:[composedCoef, composed.evalOf( val )]} ) );

			} else {

				return KhanUtil.exprToString( tackOn( base.expr( val ),
					{op:"*", params:[composedCoef, composed.name+"("+val+")"]} ) );
